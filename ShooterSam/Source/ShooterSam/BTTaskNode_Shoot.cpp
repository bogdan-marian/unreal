...
EBTNodeResult::Type UBTTaskNode_Shoot::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (OwnerController)
	{
		AShooterSamCharacter* OwnerCharacter = Cast<AShooterSamCharacter>(OwnerController->MyCharacter);
		AShooterSamCharacter* PlayerCharacter = Cast<AShooterSamCharacter>(OwnerController->PlayerCaracter);
