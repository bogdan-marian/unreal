		...
		else if (HitActor->ActorHasTag("Lock"))
		{
			...
			if (LockActor)
			{
				...
				if (!LockActor->GetIsKeyPlaced())
				{
					...
					UE_LOG(LogTemp, Display, TEXT("Items removed: %d"), ItemsRemoved);
					if (ItemsRemoved)
					{
						LockActor->SetIsKeyPlaced(true);
						UE_LOG(LogTemp, Display, TEXT("Key item removed from inventory!"));
					}
					else
					{
				...
				// 2 - do we have an item in the item list
...
void ADungeonEscape3Character::MoveInput(const FInputActionValue& Value)
{
...
void ADungeonEscape3Character::DoMove(float Right, float Forward)
{
	if (GetController())
	{
...
void ADungeonEscape3Character::DoJumpStart()
{
	// pass Jump to the character
	Jump();
}

void ADungeonEscape3Character::DoJumpEnd()
{
